...
import angel.campos.helpdesk.InfoTicket
...
class Adaptador(private var Datos: List<tbTickets>): RecyclerView.Adapter<ViewHolder>() {
    ...
    fun eleminarRegistro(TituloTicket: String, posicion: Int) {
        ...
        GlobalScope.launch(Dispatchers.IO){
            ...
            val borrarTicket = objConexion?.prepareStatement("DELETE tbTicket WHERE TituloTicket = ?")!!
    ...
    fun editarTicket(TicketModificar: tbTickets){
        GlobalScope.launch(Dispatchers.IO){
            ...
            val updateTicket = objConexion?.prepareStatement("UPDATE tbTicket SET TituloTicket =?, DescripcionTicket =?, NombreRespTicket =?, Email =?, Telefono =?, Ubicacion =? WHERE NumeroTicket =?")!!
